...
class Kalman:
    ...
    def __init__(self, initial_est, estimate_covariance,
        ...
        self.g = np.array([0, 0, -1]).T
        self.m = np.array([1, 0, 0]).T
    ...
    def update(self, gyro_meas, acc_meas, mag_meas, time_delta):
        ...
        e = np.zeros(shape=(6,), dtype=float)
        e[0:3] = acc_meas - self.estimate.rotate(self.g)
        e[3:6] = mag_meas - self.estimate.rotate(self.m)
        e = e.reshape((-1, 1))
        S = np.dot(e, e.T)
        diagS = np.diagonal(S)
        S = np.diag(diagS)
        ...
        inn_cov = H.dot(PH_T) + S + self.observation_covariance
